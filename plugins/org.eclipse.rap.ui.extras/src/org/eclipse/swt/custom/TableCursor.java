...
public TableCursor(Table parent, int style) {
	...
	// Initialise the tool tip to ensure tool tips are marked to be read
	setToolTipText(null);
	///###############  END PATCH CONTRIBUTION  ###############
...
//						case SWT.TRAVERSE_ARROW_PREVIOUS :
	...
	int[] events = new int[] {SWT.Dispose, SWT.FocusIn, SWT.FocusOut, SWT.KeyDown, /* RAP SWT.Paint,*/ SWT.Traverse};
...
void onDispose(Event event) {
	...
	try {
		...
		// [dm] to avoid unexpected exceptions
	}
	///###############  END PATCH CONTRIBUTION  ###############
}
...
///###############  END PATCH CONTRIBUTION  ###############
	...
	gc.setFont(getFont());
	gc.setBackground(row.getBackground(columnIndex));
	background = row.getBackground(columnIndex);
	gc.setForeground(row.getForeground(columnIndex));
	foreground = row.getForeground(columnIndex);
	gc.fillRectangle(getClientArea().x, getClientArea().y, getClientArea().width, getClientArea().height);
	///###############  END PATCH CONTRIBUTION  ###############
	int x = 0;
	Point size = getSize();
...
//	removeListener(SWT.DefaultSelection, listener);
	...
	for (Object registeredListener : getListeners(SWT.Selection)) {
		if (registeredListener instanceof TypedListener) {
			if (((TypedListener) registeredListener).getEventListener().equals(listener)) {
				removeListener(SWT.Selection, (Listener) registeredListener);
				removeListener(SWT.DefaultSelection, (Listener) registeredListener);
				return;
			}
		}
	}
...
// RAP
	...
	public String getToolTipText() {
		return this.row.getText(this.table.indexOf(this.column));
	}
